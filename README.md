...
- Private server-backed workspaces with HttpOnly sessions and recovery keys.
...
1. Save your workspace recovery key. It grants full workspace access; treat it like a password.
...
| Records     | Strong-consistency Netlify Blobs | Postgres JSONB records                | Atomic JSON files                |
...
- Recovery-key authentication is intentionally simple: there are no email accounts, roles, team invitations or password reset emails. Losing all sessions and the recovery key loses access to the workspace.
- OAuth, cron triggers, a plugin marketplace and SaaS billing are outside this release.
